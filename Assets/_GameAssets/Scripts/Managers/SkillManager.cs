...
public class SkillManager : NetworkBehaviour
{
    ...
    [SerializeField] private MysteryBoxSkillsSO[] _mysteryBoxSkils;
    [SerializeField] private LayerMask _groundLayer,_hillLayer;

    private Dictionary<SkillType, MysteryBoxSkillsSO> _skillsDictionery;

    private void Awake()
    {
        Instance = this;
        _skillsDictionery = new Dictionary<SkillType, MysteryBoxSkillsSO>();

        foreach (MysteryBoxSkillsSO skill in _mysteryBoxSkils)
        {
            _skillsDictionery[skill.SkillType] = skill;
        }
    }
    public void ActivateSkill(SkillType skillType, Transform playerTransform, ulong spawnerClientId)
    {
        SkillTransformDataSerializable skillTransformData = new SkillTransformDataSerializable(playerTransform.position,
            playerTransform.rotation, skillType, playerTransform.GetComponent<NetworkObject>());
    ...
    {
        if (!_skillsDictionery.TryGetValue(skillTransformDataSerializable.SkillType, out MysteryBoxSkillsSO skillData))
        {
            ...
            if (NetworkManager.Singleton.ConnectedClients.TryGetValue(spawnerClientId, out var client))
            {
                ...
                if (skillData.SkillType == SkillType.FakeBox)
                {
                    ...
                    positionDataSerializable = new PositionDataSerializable(new Vector3(
                        skillInstance.transform.position.x, groundHeight, skillInstance.transform.position.z));
    ...
    {
        if (Physics.Raycast(new Vector3(position.x, position.y, position.z),
            Vector3.down, out RaycastHit hit2, 10f, _hillLayer))
        {
            return 3f;
        }
